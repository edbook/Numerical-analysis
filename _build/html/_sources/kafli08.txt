...
-  Reikna andhverfu :math:`A`, :math:`A^{-1}`. Þá er
...
-  Gauss-eyðing fyrir :math:`n\times n` fylki krefst
   :math:`\frac 23n^3+\frac 12n^2 -\frac 76n` reikniaðgerða.
...
við eins og mögulegt er að reikna :math:`A^{-1}`.
...
forystustuðul með tölugildi jafnt og :math:`M_i`. (Þetta þýðir að ef
...
:math:`{\mathbb  R}^n`, eins og við sjáum fljótlega, og ekki endilega
...
:math:`\ell_2` staðallinn
^^^^^^^^^^^^^^^^^^^^^^^^^

Einnig kallaður evklíðska fjarlægðin, er gefinn með

.. math:: \|\mbox{${\bf x}$}\|_2 = \left( \sum_{j=1}^n x_j^2 \right)^{\frac 12} = \sqrt{\mbox{${\bf x}$}\cdot \mbox{${\bf x}$}}.
...
Almennt, ef :math:`1\leq p < \infty`, þá skilgreinum við
...
.. math::
   ...
   \underbrace{\|A\mbox{${\bf x}$}\|}_{\text{vigurstaðll}} \leq \underbrace{\|A\|}_{\text{fylkjastaðall}}
...
Einfaldastur er staðallinn sem tilheyrir :math:`\ell_\infty`, en hann
uppfyllir
...
Ef við höfum nálgun :math:`\tilde \mbox{${\bf x}$}` þannig að *leifin*
(e. residual)
:math:`\mbox{${\bf r}$}= A\tilde \mbox{${\bf x}$}- \mbox{${\bf b}$}` er
lítil, hvað getum við þá sagt um *skekkjuna* (e. error)
:math:`\mbox{${\bf e}$}= \mbox{${\bf x}$}-\tilde \mbox{${\bf x}$}`? Er
hún endilega lítil?
...
er *mjög* háð því hvaða staðal við veljum, en við höfum þó að
...
Talning á aðgerðunum við úrlausnina
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
| líne 4, :math:`l_4`, verður að :math:`l_4 - \frac ca l_2`.
...
:math:`U` ekki endilega :math:`LU=A` (sjá dæmi bls. 196). Þess í stað
fæst
...
Úrlausn :math:`A\mbox{${\bf x}$}=\mbox{${\bf b}$}`:
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
:math:`\|\cdot\|` þá er :math:`\rho(T) \leq \|T\|`, og að fyrir sérhvert
...
Samleitni Gauss-Seidel-aðferðar
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Lausn á hneppinu er því vigur :math:`\mbox{${\bf r}$}` þannig að
...
+-------------+----------------------+---------------------+---------------------------------------------------+
| :math:`n`   | :math:`x_1^{(n)}`    | :math:`x_2^{(n)}`   | :math:`\|{\mathbf x}^{(n+1)}-{\mathbf x}^{(n)}\|` |
+=============+======================+=====================+===================================================+
