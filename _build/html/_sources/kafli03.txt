...
gefna punkta :math:`(x_0,y_0),\dots,(x_m,y_m)` eða liggja nálægt
...
Matlab-forrit fyrir reiknirit Horners
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
**Athugasemd:** Hér gengur stikinn :math:`i` fyrir :math:`a_i, b_i, c_i`
og :math:`x_i` frá 1 upp í :math:`m`, en ekki frá 0 eins og í glærunum á
undan. Þetta helgast af því að í Matlab þá er fyrsta stak í vigri númer
1.

Margliðubrúun og Lagrange-form brúunarmargliðunnar
--------------------------------------------------
...
Brúunarmargliðan er ótvírætt ákvörðuð
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Setning** Brúunarmargliðan fyrir :math:`(x_0,y_0),\ldots,(x_m,y_m)` er
ótvírætt ákvörðuð. **Sönnun** Ef :math:`p(x)` og :math:`q(x)` eru tvær
...
punktar og því er :math:`r(x)` núllmargliðan samkvæmt undirstöðusetningu
algebrunnar. Þar með :math:`p(x) - q(x)` núllmargliðan,
þ.e. \ :math:`p(x) = q(x)`.

Brúunarmargliðan er til
~~~~~~~~~~~~~~~~~~~~~~~

**Setning** Til er margliða :math:`p` af stigi :math:`\leq m` þannig að

.. math:: p(x_0) = y_0, \quad \ldots \quad p(x_n)=y_n.

**Sönnun** Við notum þrepun til að sýna fram á tilvistina.
...
.. math:: p(x)=y_0\ell_0(x)+y_1\ell_2(x)+\cdots+y_m\ell_m(x),
...
Reiknum brúunarmargliðuna gegnum punktana :math:`(1,1)`, :math:`(2,3)`
og :math:`(3,6)` með Lagrange-margliðum.
...
þennan stuðul með :math:`y[x_i, \ldots, x_{i+j}]`. **Athugasemd**
Verkefnið er háð röð punktanna, þ.e. framsetningin (Newton-formið) á
margliðunni. Auðvitað er margliðan og gildin á henni alltaf þau sömu
(sbr. ótvíræðni glæru 5.12).
...
**Athugasemd:** Stærðin :math:`y[x_{n-1},x_n]` hefur komið fyrir áður
hjá okkur þegar við fjölluðum um sniðilsaðferð, sjá kafla 2.5.

Upprifjun á tilvistarsönnuninni
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Þrepunarskrefið í tilvistarsönnuninni fyrir brúunarmargliður (glæra
5.13) gefur okkur nú hvernig mismunakvótarnir nýtast okkur.
...
**Athugasemd:** Fyrir :math:`m=0` gildir að :math:`p(x) = y_0 = y[x_0]`.

Mismunakvótatöflur fyrir :math:`m=0,1`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Mismunakvótatöflur fyrir :math:`m=2`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Mismunakvótatöflur fyrir :math:`m=3`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
:math:`(2,3)` og :math:`(3,6)`. Stillum fyrst upp mismunakvótatöflu
...
Brúunarmargliður með margföldum punktum
---------------------------------------
...
fjöldi skilyrða sem við setjum á hana eru :math:`m+1`. (Athugið að
tilfellið :math:`k=m+1`, :math:`m_j=1` er það sem við skoðuðum í
köflunum hér á undan).

Tilfellin: (i) allir punktar eins og (ii) einn punktur
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

-  (i) Allir punktar eins Ef allir punktarnir eru einfaldir, þá er
   alhæfða brúunarverkefnið sama verkefni og brúunarverkefnið sem við
   leystum í kafla 5.1 og 5.3 með
...
-  (ii) Einn punktur Ef aftur á móti :math:`k = 1`, þá er lausn gefin
...
Við athugum fyrst ótvíræðni lausnarinnar með því að gera ráð fyrir að
...
Nú beitum við sams konar röksemdafærslu og í kafla 5.1 til þess að sýna
fram á tilvist á lausn, þ.e. við þrepum.
...
Við höfum því sannað eftirfarandi. **Setning** Ef gefnar eru
...
Skynsamir skiptipunktar og Chebyshev margliður
----------------------------------------------
...
leitt til mikilla frávika í :math:`p(x)`.
...
Chebyshev margliðurnar
~~~~~~~~~~~~~~~~~~~~~~

**Skilgreining** Fyrir náttúrlega tölu :math:`n` þá skilgreinum við
...
**Setning** Chebyshev margliðan :math:`T_n` hefur :math:`n` einfaldar
...
**Skilgreining** Margliða er kölluð *stöðluð* ef forystustuðull hennar
er 1.

**Skilgreining** *Stöðluðu Chebyshev margliðurnar* :math:`\tilde T` eru
skilgreindar á eftirfarandi hátt
...
**Setning** Fyrir sérhverja staðlaða margliðu :math:`q` af stigi
:math:`n` þá er
...
Skynsamlegir skiptipunktar – Skekkjumat
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
:math:`n+1`. Þannig að skv. glæru 5.48 þá lágmörkum við framlag hennar
...
**Setning** Ef :math:`q` er margliða af stigi minna en :math:`n` þá er
...
**Setning** :math:`P_n` hefur :math:`n` ólíkar núllstöðvar sem liggja
allar á :math:`[-1,1]`.

Staðlaðar Legendre margliður
~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
**Athugasemd** Setningarnar þrjár hér undan gilda um :math:`\tilde P`
alveg eins og :math:`P`.

Lágmörkun með Legendre margliðunum
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

**Setning** Ef :math:`p` er stöðluð margliða af stigi :math:`n+1` þá er
:math:`\|p\|_2\geq \|\tilde P_{n+1}\|_2`. **Sönnun** Skilgreinum
...
5.4 Athugasemd um :math:`\ell_\infty` og :math:`\ell_2`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

-  :math:`\ell_\infty` Athugið að þegar við notum :math:`\ell_\infty` og
   Chebyshev margliðurnar þá erum við að reyna að lágmarka mestu skekkju
   á bilinu.

-  :math:`\ell_2` En þegar við notum :math:`\ell_2` og Legendre
   margliðurnar þá erum við að reyna að lágmarka heildi skekkjunnar,
   þ.e. í einhverjum skilningi, flatarmálið á milli fallsins :math:`f`
   og brúunarmargliðunnar okkar.

Nálgun á föllum með margliðum og skekkjumat
-------------------------------------------
...
:math:`g` hefur að minnsta kosti :math:`m+2` núllstöðvar taldar með
margfeldni,

:math:`g'` hefur að minnsta kosti :math:`m+1` núllstöð talda með
margfeldni,

:math:`g''` hefur að minnsta kosti :math:`m` núllstöðvar taldar með
margfeldni
...
Tökum eina slíka og köllum :math:`\xi`.
...
uppfyllir sömu skilyrði og gefin eru í **a)** að viðbættu því að
...
5.1 Lausn á sýnidæmi
~~~~~~~~~~~~~~~~~~~~

**Lausn:** **a)** (og **c)**).  Til þess að spara pláss skulum við
reikna strax út mismunakvótatöfluna fyrir fjórða stigs margliðuna í
**c)**-lið. Punktarnir :math:`x_0,\dots,x_4` eru þá :math:`1,1,2,2,2` og
...
Í **a)**-lið eru punktarnir tvöfaldir svo við höfum gefin gildi
...
Í **c)**-lið er gildið á 2. afleiðu :math:`f''(x)=2\ln x +3` gefið í
...
**b)** Við stingum gildinu :math:`x=1.3` inn í margliðuna og fáum
...
hins vegar getum ekki valið þá brúunarpunktana eftir eigin höfði þá erum
...
:math:`d_i`; við krefjumst þess að:

#. :math:`S` verði samfellt tvisvar sinnum deildanlegt á öllu bilinu
   :math:`[a,b]`
...
... einhver skilyrði á :math:`c_0` og :math:`c_n`.
...
Gerum nú ráð fyrir að gefnir punktar :math:`(x_0,y_0),\dots,(x_n,y_n)`
...
Lýsing á verkefninu með línulegri algebru
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
þar sem :math:`\|\cdot\|` táknar evklíðska normið (staðalinn) á
:math:`{{\mathbb  R}}^m`.
...
Í skránni adferd\_minnstu\_fervika.m á heimasvæði okkar á Uglu finnið
þið forrit til þess að finna bestu margliðu af hvaða stigi sem er gegnum
gefið punktasafn.

Það er hægur vandi að breyta þessu forriti ef þið viljið vinna með aðra
fallagrunna en margliður.
...
þar sem grunnföllin þrjú eru :math:`f_1(x)=1`, :math:`f_2(x)=x` og
:math:`f_3(x)=x^2`.
...
.. |image| image:: ./vond_bruun1.png
.. |image| image:: ./vond_bruun2.png
.. |image| image:: ./legendre.png
.. |image| image:: ./vond_bruun3.png
.. |image| image:: ./synidaemi_minnstu_fervik.pdf
